...
;(function ($) {
  function createCode(dom, config) {
    ...
    for (i = 0; i < _this.codeLen; i++) {
      ...
      html += '<span style="padding:8px 2px;font-family: Verdana, Geneva, sans-serif;font-size: 16px;border-radius: 3px;font-weight:bold;color:' + _this.color[idx] + ';display:inline-block;transform:rotate(' + deg + 'deg);">' + _this.codeList[index] + '</span>';
      _this.code += _this.codeList[index];
      _this.bgColor = _this.bgColorList[bgIdx]

    }
    html += '<input id="idcode"  name="idcode" value="'+_this.code+'" type="hidden">';
    $(dom).css({
