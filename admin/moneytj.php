...
$dosql->Execute("SELECT *,sum(pick_number) as applymoney from `pmw_pickmoney` where pick_statues = 1 group by pick_ymd asc limit 15");
...
<form name="form" id="form" method="post" action="comment_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
        ...
        <?php
		$dopage->GetPage("SELECT *,sum(pick_number) as applymoney from `pmw_pickmoney` where pick_statues = 1  group by pick_ymd asc",15); 
		while($row = $dosql->GetArray())
		{
		$sum[]=$row['applymoney'];
      ?>
		<tr align="left" class="dataTr">
			<td height="42" align="center"><?php  echo $row['pick_ymd'];?></td>
