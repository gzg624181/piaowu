...
<body>
<div class="topToolbar"> <span class="title">充值统计&nbsp;&nbsp;&nbsp;<i class="fa fa-area-chart" aria-hidden="true"></i></span>
 <a href="javascript:location.reload();" class="reload">刷新</a></div>
<?php
date_default_timezone_set('PRC');
$dates2="";

$dosql->Execute("SELECT *,sum(chargenumber) as applymoney from `pmw_charge` group by charge_ymd asc limit 15");
while($row=$dosql->GetArray()){
    $pv[] = floatval($row['applymoney']);//提现总金额  //注意这里必须要用intval强制转换，不然图表不能显示
	  $dates2.="'".$row['charge_ymd']."',";
}
$data = array(
array(
"name"=>"充值金额",
...
$(function () {
        $('#container').highcharts({
            title: {
                text: '<?php echo $cfg_webname;?>'+ "15天充值统计表",
                x: -20 //center
            },
            subtitle: {
...
<form name="form" id="form" method="post" action="comment_save.php">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
        ...
        <?php
		$dopage->GetPage("SELECT *,sum(chargenumber) as applymoney from `pmw_charge` group by charge_ymd asc",15);
		while($row = $dosql->GetArray())
		{
		$sum[]=$row['applymoney'];
      ?>
		<tr align="left" class="dataTr">
			<td height="42" align="center"><?php  echo $row['charge_ymd'];?></td>
