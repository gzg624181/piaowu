...
if($cfg_database=="Y"){
require_once(PHPMYWIND_INC.'/conn.inc.php');     //引入本地数据库类
}else{
require_once(PHPMYWIND_INC.'/conn.inc.online.php');     //引入本地数据库类
}
...
//获取表里面最大的开奖开奖期数
function GetKj_times()
{
	global $dosql;
  $tbname='pmw_lotterynumber';
	$r = $dosql->GetOne("SELECT MAX(kj_times) AS `kj_times` FROM `$tbname`");
	$kj_times = (empty($r['kj_times']) ? 2421388 : ($r['kj_times'] + 1));
