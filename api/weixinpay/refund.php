 ...
 *
 * 需要提供的参数    refund_money  退款金额(默认全部退款)
 *                refund_orderid 订单id
 */
header('Content-type:text/html; Charset=utf-8');
require_once("../../include/config.inc.php");
//获取订单详情
$arr_order = get_order($refund_orderid);

$mchid = $cfg_mchid;   //微信支付商户号 PartnerID 通过微信支付商户资料审核后邮件发送
$appid = $cfg_appid;   //微信支付申请对应的公众号的APPID
$apiKey = $cfg_key;    //https://pay.weixin.qq.com 帐户设置-安全设置-API安全-API密钥-设置API密钥

$orderNo = $arr_order['orderid'];       //商户订单号（商户订单号与微信订单号二选一，至少填一个）
$wxOrderNo = $arr_order['transaction_id'];//微信订单号（商户订单号与微信订单号二选一，至少填一个）
$totalFee = $arr_order['totalamount'];  //订单金额，单位:元
$refundFee = $refund_money;             //退款金额，单位:元
...
if($result===true){
    ...
    //将退款的状态更改掉
    $dosql->ExecNoneQuery("UPDATE pmw_order SET refund_state=2 where orderid='$orderNo' and transaction_id='$wxOrderNo'");
    ShowMsg("退款成功！",-1);
}else{
    ShowMsg("退款失败！",-1);
}
